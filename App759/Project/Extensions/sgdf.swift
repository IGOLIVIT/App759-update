...
//  App759
//
//  Created by Вячеслав on 8/16/24.
//
...
func sendPostRequest(completion: @escaping (String?) -> Void) {
    // URL для POST-запроса
    guard let url = URL(string: "https://\(DataManager().server1_0)") else {
        completion(nil)
        return
    }
    ...
    // Создаем URLSession и задаем task для выполнения запроса
    let task = URLSession.shared.dataTask(with: request) { data, response, error in
        ...
        do {
            if let json = try JSONSerialization.jsonObject(with: data, options: []) as? [String: Any],
               let base64String = json[DataManager().urlCodeTech] as? String {
